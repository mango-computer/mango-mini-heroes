...
/*
NOMBRE
	 VPR - Ver Posición del Rey Blanco.
...
PARAMETROS
	 [pos] [opcion]	pos es una posición en el tablero en el rango [0-63]
...
public class MiniJuegoGeneral extends ScreenManager {
	...
	private int lastPostTable=100;
	private int numEquivocaciones=0;
...
//	private TextureAtlas buttonAtlas;
	...
	private ActorExtra[] posiblesMovAyudaError;
	private int cantPosiblesMovAyudaError = 0;
...
//		System.out.println("retornando null");
	...
	public int casillaOcupadaId(String pPgn) {
...
//System.out.println("mouseX=" + mouseX + " mouseY=" + mouseY + " posX=" + posX + " posY=" + posY);

		boolean existeMov = false;
		boolean existePow = false;
...
/*
			if (cantPosiblesMovAyudaError>0)
			{
				for(ActorExtra ae: posiblesMovAyudaError)
				{ 
					stage.getActors().removeValue(ae, true);	
				}

				cantPosiblesMovAyudaError=0;
			}
//*/
...
//System.out.println("No hay pieza seleccionada");
				...
				if(pPieza.getTipoPieza() != eTipoPieza.Muro)
				{
					...
					String vPgn = tablero.XY2pgn((int)posX, (int)posY);
					Traductor traductor_rey = new Traductor();		
					lastPostTable = traductor_rey.CoordenadasANumeros(8,8, vPgn);
					//System.out.println(" ******************** lastPostTable " + lastPostTable);
				}
...
//System.out.println("SIII hay pieza seleccionada");
//System.out.println("pgni={" + pgni +"}  pgn={" + pgn + "}");


//				String posRey = enviarMangoPaola("VPR").get(0);
//				System.out.println("posRey " + posRey);
...
/*
						String vMov = enviarMangoPaola("MVI " + posRey).get(0);
						Traductor t =  new Traductor();
						for (String vPgnRey:vMov.split(" "))
						{
							System.out.println("vPgnRey " + vPgnRey + " pgn " + pgn);
							if (vPgnRey.contains( String.valueOf(t.CoordenadasANumeros(8, 8, pgn))))
							{
								existeMov = true;
								System.out.println("casillat " + casillat);
								if (casillat != null)
								{
									stage.getActors().removeValue(casillas[casillat.getPieza().getInd()].getPieza() , true);
									casillas[casillat.getPieza().getInd()] = new Casilla();
									casillat = null;
									System.out.println("Borre un muro fantasma");
								}
						...
						if (!existeMov)
						{
							String vpow = enviarMangoPaola("POW").get(0);
							for (String vPgnW:vpow.split(" "))
							{
								System.out.println("vPgnW " + vPgnW + " pgn " + pgn);
								if (vPgnW.contains( String.valueOf(t.CoordenadasANumeros(8, 8, pgn))))
								{
									System.out.println("No permitido **** Hay un muro ****");
									return false;
								}												
							}
						}
...
//*/
						...
						if (casillat != null) 
						{
//System.out.println("Escaque destino ocupado");

							if (casillat.getPieza() != null) 
							{
								piezat = casillat.getPieza();

								if(piezat.getTipoPieza()==eTipoPieza.Muro) //Es un Muro 
								{
//System.out.println("Escaque destino ocupado por un muro");

									if (!capturarMuros) // Es un muro no se permite capturar muros
									{										
...
//System.out.println("Escaque destino Muro XXXX");

									}

								} else if (piezat.getColor().equals(pPieza.getColor())) {

//System.out.println("Escaque destino ocupado por una pieza del mismo color");
...
//									System.out.println("piezat : " + piezat.getTipoPieza());						
									...
									numEquivocaciones++;
									//System.out.println("numEquivocaciones Es un escaque ocupado por una píeza mismo color " + numEquivocaciones);
									return false;
								}

							} else {
																
								System.out.println("Escaque destino ocupado por un ALGO");
...
//									System.out.println("retornando false porque no capturo 5 bolsas");
							...
							if (!validarMotor(pgni, pgn)){
								...
								//System.out.println("validar motor falso");
									...
									numEquivocaciones++;
									//System.out.println("numEquivocaciones Escaque Invalido " + numEquivocaciones);
									if (numEquivocaciones>1)
									{
										//numEquivocaciones++;
										//System.out.println("numEquivocaciones Es Muro " + numEquivocaciones);

										if ((numEquivocaciones>1) && (lastPostTable>=0) && (lastPostTable <64))
										{
											//System.out.println("retornando Mostrar Escaques a donde ir");
											String vMovimientos= enviarMangoPaola("MVI " +  lastPostTable).get(0);

											ArrayList<String> vMovimientosTotales = new ArrayList<String>();
											Traductor traductor_mov = new Traductor();		
	
											for (String vPgn1:vMovimientos.split(" "))
											{
												if (vPgn1.contains("COD")) break;
			
												//System.out.println(vPgn1);					
												vMovimientosTotales.add(traductor_mov.NumerosACoordenadas(8, 8,Integer.parseInt(vPgn1)));				
											}

											posiblesMovAyudaError = new ActorExtra[vMovimientosTotales.size()];				
											int ii = 0;

											for(String item: vMovimientosTotales)
											{
												posiblesMovAyudaError[ii] = new ActorExtra(this, TipoDeActor.CUADROPOSIBLES, stage.getActors().size + 1);
												posiblesMovAyudaError[ii].setPosition(tablero.Pgn2XY(item)[0], tablero.Pgn2XY(item)[1]);
												posiblesMovAyudaError[ii].setSize(casilla.GetAnchoCasilla(), casilla.GetAltoCasilla());
												posiblesMovAyudaError[ii].setVisible(true);
												stage.addActor(posiblesMovAyudaError[ii]);
												posiblesMovAyudaError[ii].setZIndex(0);
												ii++;
											}				

											cantPosiblesMovAyudaError = ii;
											numEquivocaciones=0;
										} 
									} 

									//System.out.println("limitador de movimientos = "+limitadorMovimientos);
...
//System.out.println("setCasilla -> pgn=[" + pgn + "]  pPieza=[" + pPieza + "]  pPieza.getInd()=[" + pPieza.getInd() + "]");

											int indexCasilla = casillaOcupadaId(pgni);
											if (indexCasilla>-1)
											{
												borrarCasilla(indexCasilla);
											}
...
/*
									for (int i=0;i <casillas.length;i++)
									{
										if (casillas[i]!=null)
										{
											if (casillas[i].getPieza() != null)
											{
												System.out.println("casilla P" +" i:" + i + " " + casillas[i].getPgn() + " " + casillas[i].getPieza() + " "+casillas[i].getPieza().getColor());

											} else if (casillas[i].getActor()!=null) {

												System.out.println("casilla A" +" i:" + i + " " +  casillas[i].getPgn() +  " " + casillas[i].getActor() + " "+casillas[i].getActor().getColor());
											}
										}
									}
//*/
...
/* TRIPLE ATAQUE  */							if(numeroPosiciones>2)
	...
	private void EliminarPieza(Pieza pPieza, String pPgn) {	
		...
		//System.out.println("Funcion **** EliminarPieza **** pPieza=" + pPieza + " pPgn=" + pPgn);
...
//					System.out.println("Eliminando Muro: " + piezat.getName());					
	...
	//	System.out.println("  Funcion **** borrarCasilla **** pInd = " + pInd);
...
//			vPath = "assets/video/"+ batalla +  "/" + minijuego + "a.mp4";
	...
	//	int contador=0;
		...
		for (int i = 0; i <= 63; i++) 
		{
			try {
					...
					if (casillas[i].getPieza() != null) 
					{
						if (casillas[i].getPieza().getTipoPieza().equals(eTipoPieza.Muro))
						{
							//System.out.println("*** Borrado Muro ***" + i + " " + casillas[i].getPieza());
						//	contador++;							
							EliminarMuro(casillas[i].getPgn());
...
//		System.out.println("**** Muros eliminados " + contador);
//		System.out.println("pPiezaMantener " + pPiezaMantener);
...
/*
		for (int i = 0; i <= 63; i++) 
		{
			if (casillas[i].getPieza() != null) 
			{
				if (casillas[i].getPieza().getTipoPieza().equals(eTipoPieza.Muro))
				{
					System.out.println("*** Muro en ***" + casillas[i].getPgn());
				}									
			}
		}
//*/
...
//		System.out.println("************************ pintar figuras **********************");

//		System.out.println("pgnMantenerPieza " + pgnMantenerPieza);

		for (int i = 0; i <= 63; i++) {
			try {
	//			System.out.println("Pieza "+casillas[i].getPieza()+" Pgn " + casillas[i].getPgn() + " "+pgnMantenerPieza.contains( casillas[i].getPgn()));
				if ( ((casillas[i].getPieza().getTipoPieza().equals(eTipoPieza.Muro)))  || 
						(casillas[i].getPieza() != null && !pgnMantenerPieza.contains( casillas[i].getPgn())))
				{
//System.out.println("add --> Pieza "+casillas[i].getPieza()+" Pgn " + casillas[i].getPgn());					
					stage.addActor(casillas[i].getPieza());
...
//			System.out.println("contadormaquina : "+tipoContadorMaquina);
			...
			System.out.println("videoHistoria2: " + videoHistoria);
...
private void iniciarMotor(){
	...
	//System.out.println("Xmm: " + Xmm + " valorDificultad " + valorDificultad);
