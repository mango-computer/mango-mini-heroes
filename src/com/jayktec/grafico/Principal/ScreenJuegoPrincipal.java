...
import com.jayktec.grafico.ImagenTurnoJugador;
...
public class ScreenJuegoPrincipal extends ScreenManager {
	...
	private long segEntreJugadasBlancas=-1;
	private long segEntreJugadasNegras=-1;
	

	private	boolean esPasado = false;
	private	boolean esFlechaVisible = false;

	private Process mplayerProcess; 
	private boolean varJugarNow = false;

	final ImagenTurnoJugador imgTurnoJugador;

	private Image labelBlancas;
	private Image labelNegras;

	private long lngTiempoMangoPaola=0;
	...
	private Stage stage;
	private TextButton salir, reiniciar, cambioPieza, juegaMango, deshacer, Opcion, jugarNow; //Guardar
...
//	public ArrayList<Casilla[]> listaCasillasDeshacerp= new ArrayList<Casilla[]>();
	...
	public String valorDificultad = "sd 3";
...
//	private Texture textureCuadro;
	...
	private int jugadaRevisarHistorico = 0 ;
...
//	private boolean ultimaJugadaGuardada = false;
	...
	private Color colorAntesDelBlue = Color.BLUE;
	...
	//	font.setColor(0,0,0,1);
		...
		imgTurnoJugador = new ImagenTurnoJugador(pGame);
...
//System.out.println(">>>>>>>>>>>>>>>>> selectTipoPieza " + selectTipoPieza);
//System.out.println(">>>>>>>>>>>>>>>>> tipoDePiezas " + tipoDePiezas);
//System.out.println(">>>>>>>>>>>>>>>>> jugadas " + jugadas.size());
//System.out.println(">>>>>>>>>>>>>>>>> jugadasAlg " + jugadasAlg.size());
//System.out.println(">>>>>>>>>>>>>>>>> listaCasillasDeshacer " + listaCasillasDeshacer.size());
...
//*
		stage.addActor(imgTurnoJugador);
        imgTurnoJugador.setSize(18,18);
		imgTurnoJugador.setZIndex(0);
		imgTurnoJugador.setVisible(true);
		imgTurnoJugador.setTurnoBlanco(colorOponente);
//*/
...
//		
		...
		if (historia){
			...
			if(selec4==null){
				selec4="5 min";
//System.out.println("Comprobar Tiempo 1");
				comprobartiempo();
			}
		...
		//System.out.println(">>>>>>>>>>>>>>>>> XXX jugadas " + jugadas.size());
		//System.out.println("********** x E N T R A N D O x ********************* ");
	...
	if (tocarCortinas)
	{
		...
		if (mplayerProcess==null)
		{
			try {
				...
				mplayerProcess = Runtime.getRuntime().exec("mplayer -loop 0 -slave -quiet -idle assets/Sonidos/cortinaFondo.ogg");
				System.out.println("mplayerProcess ON");


			} catch(IOException ioe) {System.out.println("*** MPLAYER EXCEPTION nuevo ***" + ioe.toString());}
		}
...
/*	
		if(mostrarcontTiempo){
System.out.println("Comprobar Tiempo 2");
			comprobartiempo();
			}
//*/
		jugadasSecretasSeguidas=0;
...
if(mostrarcontTiempo){
			//System.out.println("este es select "+selec4);
//System.out.println("Comprobar Tiempo 3");
			comprobartiempo();
			inicializarRelojes();
			oprimirBotonReloj();
			}
...
//enviarMangoPaola("setboard  " + vStringFen);
	...
	private void cargarPersonaje() {
		...
		try {
			...
			videoHistoria = videoHistoria.replace(".ogv",".mp4");

			System.out.println("videoHistoria1: " + videoHistoria);
...
//	}
	...
	private void retardoMangoPaola()
	{
	...
	public void render(float delta) 
	{
		...
		//System.out.println(">>>> mostrarcontTiempo " + mostrarcontTiempo);

		if(mostrarcontTiempo && (skin5!=null))
		{
			if(timecount>=1)
			{
...
/*
				if(flagPane )
				{
					System.out.println("ScrollTo 20");
...
//*/					
				...
				if (jugandovsMangoPaola) 
				{
					try {
						...
						{
							//System.out.println("XXXX 1 XXXX");
							if (!bGanar && !primerajugada)
							{

//								System.out.println("XXXX 2 XXXX");

								JugarMangoPaola();

								//***************
								if(mostrarcontTiempo && (skin5!=null) && (timecount>=1))
								{
									if(getTurno().equals(Color.BLACK))
									{
										determinarTiempoBla();
					...
					} catch (Exception e) { System.out.println("jugar vacio");}
				
				} else { //else de jugandovsMangoPaola
...
/*
					if (!Turno.equals(colorOponente)^tablero.getTableroRotado()) 
					{
						Timer.schedule(new Task() {
							...
							public void run() 
							{
								System.out.println("**** 1 deshacer.setTouchable(Touchable.enabled)");
								deshacer.setTouchable(Touchable.enabled);
							}
						}, 0.5f * 1);
...
//*/
				} // end if jugandovsMangoPaola

			} //end if conversacion.isArriba()
			...
			if (jugar())
			{
				if (bGanar)
				{
					...
					if(mostrarcontTiempo && (skin5!=null))
					{
						//System.out.println("YYYY 1 AZUL YYYY");
						colorAntesDelBlue = getTurno();
						setTurno(Color.BLUE);
					}
					...
					if(mostrarcontTiempo && (skin5!=null))
					{
						//System.out.println("YYYY 2 AZUL YYYY");
						colorAntesDelBlue = getTurno();
						setTurno(Color.BLUE);
					}
					...
					if(mostrarcontTiempo && (skin5!=null))
					{
						//System.out.println("YYYY 3 AZUL YYYY");
						colorAntesDelBlue = getTurno();
						setTurno(Color.BLUE);
					}
					tablas();
...
//			System.out.println(" Jugando: " + colorActivas.toString());
	...
	public boolean ValidarMovimiento(Pieza pPieza, int pJuego, int mouseX, int mouseY, float actorX, float actorY,
		...
		jugarNow.setTouchable(Touchable.disabled);
		removerFlechas();
		if (esPasado) return false;


//		jugarNow.setTouchable(Touchable.enabled);

		//System.out.println("****ValidarMovimiento **** Turno " + Turno + " **** ValidarMovimiento ****");
		//System.out.println("****ValidarMovimiento **** pPieza " + pPieza + " **** ValidarMovimiento ****");
		//System.out.println("****ValidarMovimiento **** pPieza.getColor() " + pPieza.getColor() + " **** ValidarMovimiento ****");
								...
								if(jaqueMate||tablas){
									if (!jugandovsMangoPaola&&!historia){
										if (jaqueMate){
											colocarJugadaAlg(pgni, pgn);
											if(getTurno().equals(Color.WHITE)){
												//System.out.println(">>> ganartiempoX >> 1");
												ganartiempo(true);
											}
											else{								
												//System.out.println(">>> ganartiempoX >> 2");
												ganartiempo(false);
											}							
										}else{
									bTablas=true; 
...
//								}	
						...
						if (pPieza.getTipoPieza() == eTipoPieza.Torre) {
							if (juego == 0) {
								...
								{
									//System.out.println("**** 1 deshacer.setTouchable(Touchable.disabled) ****");
									deshacer.setTouchable(Touchable.disabled);
...
//									deshacer.setTouchable(Touchable.disabled);
									...
									//System.out.println("**** B deshacer.setTouchable(Touchable.enabled) ****");
									deshacer.setTouchable(Touchable.enabled);
						...
						if (pPieza.getTipoPieza() == eTipoPieza.Alfil) {
							if (juego == 0) {
								...
								{
									//System.out.println("**** 2 deshacer.setTouchable(Touchable.disabled) ****");
									deshacer.setTouchable(Touchable.disabled);
...
//									deshacer.setTouchable(Touchable.disabled);
									...
									//System.out.println("**** C deshacer.setTouchable(Touchable.enabled) ****");
									deshacer.setTouchable(Touchable.enabled);
						...
						if (pPieza.getTipoPieza() == eTipoPieza.Reina) {
							if (juego == 0) {
								if (((Reina) pPieza).Validar(mouseX, mouseY, (int) actorX, (int) actorY, (int) rangoX,
										(int) rangoY, (int) posX, (int) posY) == true) 
								{
									//System.out.println("**** 3 deshacer.setTouchable(Touchable.disabled) ****");
									deshacer.setTouchable(Touchable.disabled);
...
//									deshacer.setTouchable(Touchable.disabled);
									...
									//System.out.println("**** D deshacer.setTouchable(Touchable.enabled) ****");
									deshacer.setTouchable(Touchable.enabled);
						...
						if (pPieza.getTipoPieza() == eTipoPieza.Rey) {
							if (juego == 0) {
										...
										(int) rangoY, (int) posX, (int) posY) == true) 
								{
									...
									//System.out.println("**** 4 deshacer.setTouchable(Touchable.disabled) ****");
									deshacer.setTouchable(Touchable.disabled);
...
//									deshacer.setTouchable(Touchable.disabled);
									...
									//System.out.println("**** E deshacer.setTouchable(Touchable.enabled) ****");
									deshacer.setTouchable(Touchable.enabled);
						...
						if (pPieza.getTipoPieza() == eTipoPieza.Caballo) {
							if (juego == 0) {
										...
										(int) rangoY, (int) posX, (int) posY) == true) 
								{
									...
									//System.out.println("**** 5 deshacer.setTouchable(Touchable.disabled) ****");
									deshacer.setTouchable(Touchable.disabled);
...
//									deshacer.setTouchable(Touchable.disabled);
									...
									//System.out.println("**** F deshacer.setTouchable(Touchable.enabled) ****");
									deshacer.setTouchable(Touchable.enabled);
						...
						if (pPieza.getTipoPieza() == eTipoPieza.Peon) {
							if (juego == 0) {
										...
										(int) rangoY, (int) posX, (int) posY) == true) 
								{
									...
									//System.out.println("**** 6 deshacer.setTouchable(Touchable.disabled) ****");
									deshacer.setTouchable(Touchable.disabled);
...
//									deshacer.setTouchable(Touchable.disabled);
									...
									//System.out.println("**** A deshacer.setTouchable(Touchable.enabled) ****");
									deshacer.setTouchable(Touchable.enabled);
...
//				piezat.remove();
	...
	private void JugarMangoPaola() {
		...
		if (!jaqueMate && !tablas) {
			...
			//System.out.println("**** JugarMangoPaola DISABLE OFF OFF BOTONES ****");
			deshacer.setTouchable(Touchable.disabled);
			jugarNow.setTouchable(Touchable.disabled);

			juegaMango.setTouchable(Touchable.disabled);
			cambioPieza.setTouchable(Touchable.disabled);
			reiniciar.setTouchable(Touchable.disabled);
			salir.setTouchable(Touchable.disabled);
			Opcion.setTouchable(Touchable.disabled);
			botonjugAyuda.setTouchable(Touchable.disabled);

			// int i = 0 ;
			//System.out.println("*********** >>1<< Juega Mango Paola ****************");
		...
		if (peonMovimiento != null) {
			...
			if(mostrarcontTiempo)
			{
				...
				lngTiempoMangoPaola = (hora2.getTime()-hora1.getTime());
				if (lngTiempoMangoPaola<800)
				{
					lngTiempoMangoPaola = (long)0;

				} else {

					lngTiempoMangoPaola = (long)Math.rint((double)((double)lngTiempoMangoPaola/1000));
				...
				varRes = resultadoJugarMangoPaola(enviarMangoPaola("go"));
				lngTiempoMangoPaola = 0;
			}

			//System.out.println("*********** >>2<< Juega Mango Paola  ****************");


			//System.out.println("*********** lngTiempoMangoPaola " + lngTiempoMangoPaola);
			//System.out.println("*********** mostrarcontTiempo " + mostrarcontTiempo);


			if(mostrarcontTiempo && (lngTiempoMangoPaola>1))
			{
				if(getTurno().equals(Color.WHITE) && primerajugada)
				{
					determinarTiempoBla();
...
//***
/*
					if (varJugarNow) //La jugada vino de presionar el boton Jugar Ya
					{
...
//*/
		...
		//	obtenerFen();
			...
			//System.out.println("ScrollTo 10");
			pane.scrollTo(0,0 ,0, 0);
			...
			Timer.schedule(new Task(){@Override public void run() {
				//System.out.println("**** 2 deshacer.setTouchable(Touchable.enabled)");
				deshacer.setTouchable(Touchable.enabled);
				}}, 0.5f * duracion);
				...
				if (!peonpromovido)
					...
					if(casillaEstaOcupada(pgnF))
					{
				...
				else 
						{
							...
							System.out.println("Error, no seteo la casilla por que promovi un peon.. segun...");
						}
...
//			}		
			...
			//System.out.println("bTablas:"+bTablas);
			
			if (!bTablas && !bGanar && !bPerder)
			{
		...
		//System.out.println("**** JugarMangoPaola ENABLE ON ON BOTONES ****");

		//System.out.println("*** 2 JugarMangoPaola -> jugadasAlg size: " + jugadasAlg.size()); 
		...
		botonjugAyuda.setTouchable(Touchable.enabled);
...
//		System.out.println("Jugador Moviendo " + pPgnI + " - " + pPgnF);
		...
		//System.out.println("jugadaHumano "+jugadaHumano);
		//System.out.println("jugadaHumano size"+jugadaHumano.size());
	...
	private boolean resultadoValidarMotor(ArrayList<String> pRes) {
		...
		try {
			...
			for (String item : pRes) {
			 ...
			 //System.out.println("resultado Mango Paola exception " +  e.getMessage());
		}
		return true;
	}
...
//						System.out.println("cPiezaPromovida: " + cPiezaPromovida);															
		...
		dialogo.setWidth(300); 			// 360	
		dialogo.setHeight(300);			// 400
...
//		System.out.println("jugarPiezaPromovida in: " + pPgnf);
	...
	@Override
	public void pause() {
		//System.out.println("public void pause()");
...
//		System.out.println();
	...
	//System.out.println("******************************ENTRANDO CambiarTurno******************************");
	//System.out.println("********************************************************************");
	//System.out.println("********************************************************************");
	...
	//System.out.println("*** jugadasAlg-->: [" + jugadasAlg + "]");
	//System.out.println("*** jugadasAlg size: " + jugadasAlg.size()); 

	//System.out.println("*** jugadas-->: [" + jugadas  + "]");
	//System.out.println("*** jugadas size-->: "    + jugadas.size());

	//System.out.println("*** Turno: " + Turno);  
	//System.out.println("*** listaCasillasDeshacer size: " + listaCasillasDeshacer.size());  
...
//		if (tablas) return;
		...
		if (Turno == Color.WHITE)
		{
			...
			if(mostrarcontTiempo)
			{
				//System.out.println("-2- Blancas - Tiempo");
			...
			} else { //No mostrar tiempo

				//System.out.println("-3- Blancas - NO tiempo");
						
				habilitarPiezas(eColores.Negras);
				Turno = Color.BLACK;
//				imgTurnoJugador.setTurnoNegro(colorOponente);
			}

			imgTurnoJugador.setTurnoNegro(colorOponente);


		} else if (Turno == Color.BLACK) { //Turno de las negras
					
			//System.out.println("-4- NEGRAS");
			...
			if(mostrarcontTiempo)
			{
				//System.out.println("-4- NEGRAS - Tiempo");
				...
				//System.out.println("-5- NEGRAS - NO Tiempo");
...
//				imgTurnoJugador.setTurnoBlanco(colorOponente);
		...
		//System.out.println("******************************SALIENDO CambiarTurno******************************");
		//System.out.println("********************************************************************");
		//System.out.println("********************************************************************");
...
//*************************************************************************************************************************
	...
	/**
	 * deshace las ultimas dos jugadas realizadas por el usuario y el motor
	 */
	...
	public void deshacer() 
	{
		...
		//System.out.println("******************************ENTRANDO deshacer******************************");
		//System.out.println("********************************************************************");
		//System.out.println("********************************************************************");

		//System.out.println("*** Turno-->: [" + Turno + "]");
		//System.out.println("*** colorAntesDelBlue-->: [" + colorAntesDelBlue + "]");
		...
		//System.out.println("*** colorOponente-->: [" + colorOponente + "]");
		...
		//System.out.println("*** casillas size: " + casillas.length);
		...
		if (listaCasillasDeshacer.size() > 1) 
		{
			...
			if (jaqueMate || tablas) 
			{
				//System.out.println("cambiando a false el jaqueMate");

				if (tablas) //Tapando errores de logica de programacion, arreglar todo llevaria mucho tiempo
				{
					if (listaCasillasDeshacer.size() > (jugadas.size()+1))
					{
				...
				{
					if (colorAntesDelBlue.equals(Color.WHITE))
					{
						Turno = Color.BLACK;
			...
			if (jugadas.size()>0)
			{
				...
				//System.out.println("******************* INDEX OUT jugadas************************");
				//System.out.println("******************* INDEX OUT jugadas************************");
				//System.out.println("******************* INDEX OUT jugadas************************");
				//System.out.println("******************* INDEX OUT jugadas************************");
				//System.out.println("******************* INDEX OUT jugadas************************");
			...
			if (jugadasAlg.size()>0)
			{
				if (Turno.equals(Color.BLACK)) //Se elimino la ultima jugada negra
				{
					String temp= jugadasAlg.get(jugadasAlg.size()-1).split(" ")[0];
					//System.out.println("Se elimino la ultima jugada negra ----> [" + temp + "]");

					jugadasAlg.remove(jugadasAlg.size()-1);
					jugadasAlg.add(tamano(temp,15));

				} else { // Se elimino la ultima jugada blanca

					jugadasAlg.remove(jugadasAlg.size()-1);	//Un item son dos movimientos
				}

			} else {

				//System.out.println("******************* INDEX OUT jugadasAlg************************");
				//System.out.println("******************* INDEX OUT jugadasAlg************************");
				//System.out.println("******************* INDEX OUT jugadasAlg************************");
				//System.out.println("******************* INDEX OUT jugadasAlg************************");
				//System.out.println("******************* INDEX OUT jugadasAlg************************");
			...
			//System.out.println("fen despues de remover: " + enviarMangoPaola("fen"));

		}

		if (listaCasillasDeshacer.size() < 2) 
		{
			//System.out.println("*****************************************if (listaCasillasDeshacer.size() < 2)");
			//System.out.println("**** 7 deshacer.setTouchable(Touchable.disabled) ****");
		...
		TextButtonStyle stybton = new TextButtonStyle();
		stybton.font=font;

		if (jugandovsMangoPaola) 
		{
			stybton.up = skin1.getDrawable("jvjUp");
			stybton.down = skin1.getDrawable("jvjDown");
			stybton.over = skin1.getDrawable("jvjOver");
			juegaMango.setStyle(stybton);
		
		} else {
			
			stybton.up = skin1.getDrawable("jvcUp");
			stybton.down = skin1.getDrawable("jvcDown");
			stybton.over = skin1.getDrawable("jvcOver");
			juegaMango.setStyle(stybton);
			
		}


		//System.out.println("******************************SALIENDO deshacer******************************");
		//System.out.println("********************************************************************");
		//System.out.println("********************************************************************");
...
//		duracion = 1;
		...
		//System.out.println("********************************************************************");
		//System.out.println("********************************************************************");
//System.out.println("********** >>>> public void sonido --> duracion= [" + duracion + "] ********************");
		//System.out.println("********************************************************************");
		//System.out.println("********************************************************************");
		//System.out.println("********************************************************************");
			...
			if (!jugandovsMangoPaola) {
				if (!Turno.equals(colorOponente)^ tablero.getTableroRotado()) {
					Timer.schedule(new Task() {
						@Override
						public void run() {
							System.out.println("**** 3 deshacer.setTouchable(Touchable.enabled)");
							deshacer.setTouchable(Touchable.enabled);
						}
					}, 0.5f * duracion);
				} else {
					Timer.schedule(new Task() {
						@Override
						public void run() {
							System.out.println("**** 8 deshacer.setTouchable(Touchable.disabled) ****");
							deshacer.setTouchable(Touchable.disabled);
						}
					}, 0.5f * duracion);
				}
...
//		System.out.println("string del fen" + vStringFen);
		...
		if (vStringFen == null || vStringFen.equals("") || vStringFen.equals("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 0"))
		{
			juegoNuevo = true;
			...
			try {
				if (casillas[i].getPieza() != null) {
					stage.addActor(casillas[i].getPieza());
					 //System.out.println(casillas[i].getPieza().toString() + " - " + casillas[i].getPgn());
				}
...
//System.out.println("llegue aqui");
		...
		if (juegoNuevo)
		{
			...
			} else if (selec.equals("Nivel 3")) {

					random =5;

			} else if (selec.equals("Nivel 4")) {

					random =3;

			} else if (selec.equals("Nivel 5")) {
...
//			else{
	...
	private void botones() {
		...
		pane.setFlickScroll(false); //false
...
//            if (itemSeleccionado == listaDeJugadas.getSelectedIndex())
			{
            	//System.out.println("CantidadjugadaRevisar " + CantidadjugadaRevisar);
 //           	if(deshacer.isTouchable())
				{

    				jugadaRevisar = listaDeJugadas.getSelectedIndex();
					jugadaRevisarHistorico = ((jugadaRevisar+1)*2);

					if ((listaCasillasDeshacer.size() % 2) == 0)
					{
						jugadaRevisarHistorico--; 
					}

        			if (!historia)
					{		
						//System.out.println(" colocarFlechas() 1");
        				colocarFlechas();
	            	}
...
//		TextButtonStyle stycargarFen 		= new TextButtonStyle();
		...
		TextButtonStyle styjugarNow 		= new TextButtonStyle();
...
//		stycargarFen.font		=font;
		...
		styjugarNow.font		=font;
		stycambiarPieza.font	=font;
		
		styjugarNow.up 		= skin1.getDrawable("jugarNowUp");
		styjugarNow.down 	= skin1.getDrawable("jugarNowDown");
		styjugarNow.over 	= skin1.getDrawable("jugarNowOver");
...
//		stycargarFen.over = skin.getDrawable("cargarOver");
		...
		jugarNow 		= new TextButton("", styjugarNow);
...
//		Guardar 		= new TextButton("", styguardar);
		...
		jugarNow.addCaptureListener(new ChangeListener() {
			@Override
			public void changed(ChangeEvent event, Actor actor) {

				if (!esPasado && jugar())
				{
					//System.out.println("Juega Now");
					varJugarNow = true;
					JugarMangoPaola();
					varJugarNow = false;

					//Cambiar a Humano Vs Computer
		...
		Opcion.addCaptureListener(new ChangeListener() {
			...
			public void changed(ChangeEvent event, Actor actor) {
				...
				//System.out.println("*** Boton: Opcion ***");
...
//*/
		salir.addCaptureListener(new ChangeListener() {
			...
			public void changed(ChangeEvent event, Actor actor) {
				...
				//System.out.println("Salir historia" + historia);
...
/*					
					...
					if(mostrarcontTiempo){
					System.out.println("Comprobar Tiempo 4");
					comprobartiempo();
					}
//*/
...
//				game.setScreen(game.juegoPrincipal);
		...
		cambioPieza.addCaptureListener(new ChangeListener() {
			...
			public void changed(ChangeEvent event, Actor actor) {
				...
				//System.out.println("*** Boton: cambioPieza ***");
				if (!esPasado)
				{
					if (cambioPiezaPermitido && jugar())
					{
	...
	//*
						if (mostrarcontTiempo)
						{
							...
							//System.out.println("***cambioPieza.addCaptureListener***");
	...
	//							actualizarContadorT(cont3, minutos);
								...
								actualizarContadorT(cont2, tiempoJugadasBla);
								actualizarContadorT(cont1, minutos);
	...
	//							actualizarContadorT(cont1, minutosN);
								...
								actualizarContadorT(cont4, segN);
								actualizarContadorT(cont3, minutosN);
	...
	//							actualizarContadorT(cont1, minutosN);
								...
								actualizarContadorT(cont4, tiempoJugadasNe);
								actualizarContadorT(cont3, minutosN);
	...
	//*/		
		...
		juegaMango.addCaptureListener(new ChangeListener() {
			...
			@Override
			public void changed(ChangeEvent event, Actor actor) {
                //System.out.println("*** Boton: juegaMango ***"); 
...
//						System.out.println("seleccion : " + selec);
						...
						if (selec.equals("Amelia Hernández")) {
							valorDificultad = "sd 2";
							hard= false;
							dificulty = selec;
							random=0;
						}
						else if (selec.equals("Saraí Sánchez")) {
						...
						ultimaJudada ="";
						//game.setScreen(game.juegoPrincipal);
						//System.out.println("valorDificultad=" + valorDificultad + "  random="+random);
						resultadoJugarMangoPaola(enviarMangoPaola(valorDificultad));
						resultadoJugarMangoPaola(enviarMangoPaola("random "+ random));
				
						if (random>0)
						{
							resultadoJugarMangoPaola(enviarMangoPaola("book 0"));

						} else {

							resultadoJugarMangoPaola(enviarMangoPaola("book 1"));
						}
...
String[] comboPieza = new String[] {"Clásica", "MiniHéroe"};
...
String[] combotiempo = new String[] {"5 min","10 min","20 min","30 min","60 min","Ilimitado", "Personalizado"};
...
public void changed(ChangeEvent event, Actor actor) {
	...
	box4.setSelected(selec4);
//System.out.println("Comprobar Tiempo 5");
	comprobartiempo();
	...
	if (!selec4.equals("Ilimitado")) 
	{
		...
		TextButtonStyle style=	botonesReloj(cc);
...
/*		        
	  ...
	  cambioPieza.setPosition(100, 500);
	     ...
	     jugarNow.setPosition(680, 500);

	botonjugAyuda.setPosition(303, 500);
	       Opcion.setPosition(407, 500); 
	    reiniciar.setPosition(507, 500); 
	    btnTablas.setPosition(595, 500); 
	        salir.setPosition(770, 500); 
//*/			

	    reiniciar.setPosition(0, 500); 
         juegaMango.setPosition(100, 500);
	     jugarNow.setPosition(200, 500);
	     deshacer.setPosition(303, 500);
	  cambioPieza.setPosition(407, 500);
	    btnTablas.setPosition(507, 500); 
	       Opcion.setPosition(595, 500); 
	botonjugAyuda.setPosition(680, 500);
...
//		Guardar.setTouchable(Touchable.enabled);
		...
		if (!activaDeshacer) {
			//System.out.println("**** 9 deshacer.setTouchable(Touchable.disabled) ****");
			deshacer.setTouchable(Touchable.disabled);
			activaDeshacer = true;
		}
		if(jugandovsMangoPaola){
...
//		System.out.println(">>>>>>>>>>>>>> FUNCION cambioDePiezas " + historia + " " + selectTipoPieza);
...
//		System.out.println("**************** >>>>>>>>>>>>>> ***************** cambio tipo de pieza " + tipoPieza + " len " + casillas.length);
		for (int i=0 ; i<casillas.length;i++)
		{
			if (!(casillas[i].getPieza()==null))
			{
//				System.out.println(" casillas[i].getPieza() " + casillas[i].getPieza());
...
System.out.println("Hola Mundo desde aqui...");
...
//System.out.println("entre a preferencias");
		...
		if (preferencias.getPreferencia("musica")) {
			...
			if (mplayerProcess==null)
			{
				...
				} catch(IOException ioe) {System.out.println("*** MPLAYER EXCEPTION preferencias ***" + ioe.toString());}
...
//					game.setScreen(game.juegoPrincipal);
					...
					jugadasAlg = new ArrayList<String>();
					listaDeJugadas.setItems(jugadasAlg.toArray());
					//jaqueMate = false; esta demas, en nuevaPartida se repite 2 veces, es una locura de codigo
					nuevaPartida();
					...
					if(mostrarcontTiempo){
//System.out.println("Comprobar Tiempo 6");
						comprobartiempo();
						}
				...
				if (historia){ 
					...
					if(mostrarcontTiempo){
						//System.out.println("comprobarTiempo AZUL");
						//comprobartiempo();
						colorAntesDelBlue = getTurno();
						setTurno(Color.BLUE);
						}
...
//System.out.println("Comprobar Tiempo 7");
							comprobartiempo();
							//System.out.println("YYYY AZUL 10 YYYY");
							colorAntesDelBlue = getTurno();
							setTurno(Color.BLUE);
							}
					}
...
//				game.setScreen(game.video);
	...
	private void jugadaAyuda(){
		...
		//System.out.println("************ jugadaAyuda -> DISABLE BOTONES *****");
...
//	    System.out.println("vPgn1:"+ vPgn1);
		...
		//System.out.println("************ jugadaAyuda -> ENABLE BOTONES *****");
...
//*/
	...
	//System.out.println("YYYY AZUL 20 YYYY");
...
//							game.setScreen(game.mapGen);
	...
	private void cargarhistoria() {
		try{	
			...
			System.out.println("Error cargando el video Historia: " + videoHistoria);		
		}
	...
	private void cargarBiografia() {	
		try{
			if (!videoBiografia.equals("0"))
				...
				System.out.println("Error, no se ha configurado el video de Biografia para este minijuego");
		}
		catch(Exception e){
			System.out.println("Error cargando el video Biografia: " + videoHistoria);	
			}
		}
/**
...
//				System.out.println("entre aqui 2");
	...
	if (!jaqueMate&&!tablas){
		//System.out.println("**** 4 deshacer.setTouchable(Touchable.enabled)");
		deshacer.setTouchable(Touchable.enabled);
	} else {		
...
//		skin3.addRegions(pack);
	...
	private void colocarFlechas(){
		...
		if (esFlechaVisible) return;

		esFlechaVisible = true;

		//System.out.println(" private void colocarFlechas() ");

		CantidadjugadaRevisar = listaDeJugadas.getItems().size;

		if ((jugadaRevisarHistorico+1)>=listaCasillasDeshacer.size())
		{
			esPasado = false;
			...
			if (Turno==Color.WHITE)
			{				
				imgTurnoJugador.setTurnoBlancoCandado(colorOponente);

			} else {

				imgTurnoJugador.setTurnoNegroCandado(colorOponente);
			}
...
//					System.out.println(casillas[i].getPgn());
	...
	private void revisar2(int pAvanzar) {
		
		//System.out.println("revisando2: " + pAvanzar + " jugadaRevisar " + jugadaRevisar + " CantidadjugadaRevisar: " + CantidadjugadaRevisar + " listaCasillasDeshacer.size() " + listaCasillasDeshacer.size());
		//System.out.println("revisando2: jugadaRevisar " + jugadaRevisar);
		//System.out.println("revisando2: jugadaRevisarHistorico " + jugadaRevisarHistorico);

		//System.out.println("revisando2: CantidadjugadaRevisar: " + CantidadjugadaRevisar);
		//System.out.println("revisando2:  listaCasillasDeshacer.size() " + listaCasillasDeshacer.size());

		CantidadjugadaRevisar = listaDeJugadas.getItems().size;

//		System.out.println("**** 10 deshacer.setTouchable(Touchable.disabled) ****");
		deshacer.setTouchable(Touchable.disabled);
		flagPane = false;				
	

		if (pAvanzar==1) { //Avanzar

			jugadaRevisarHistorico++;

			if (jugadaRevisar < CantidadjugadaRevisar) {

				if ((jugadaRevisarHistorico % 2) == 1)
				{
					jugadaRevisar++;
					//System.out.println("ScrollTo 3");
					if (jugadaRevisar<CantidadjugadaRevisar)
					{
						listaDeJugadas.setSelectedIndex(jugadaRevisar);
					}
		...
		else if (pAvanzar==0) { //Retroceder

			jugadaRevisarHistorico--;

			if (jugadaRevisar > 0) {

				if ((jugadaRevisarHistorico % 2) == 0)
				{
					jugadaRevisar--;
					//System.out.println("ScrollTo 2");
					if (jugadaRevisar<CantidadjugadaRevisar)
					{
						listaDeJugadas.setSelectedIndex(jugadaRevisar);
					}
		...
		} else if (pAvanzar==2) { //Click

			//System.out.println("ScrollTo 1");
			if (jugadaRevisar<CantidadjugadaRevisar)
			{
				listaDeJugadas.setSelectedIndex(jugadaRevisar);
			}
		...
		//System.out.println("1**************************: jugadaRevisarHistorico " + jugadaRevisarHistorico);
...
//*/
		int limite = listaCasillasDeshacer.size()-1;

		if (jugadaRevisarHistorico>limite)
		{
			jugadaRevisarHistorico = limite;
			removerFlechas();
			deshacer.setTouchable(Touchable.enabled);

		} else if (jugadaRevisarHistorico<0) {

			jugadaRevisarHistorico = 0;
		}

		//System.out.println("2*********************** ** jugadaRevisarHistorico " + jugadaRevisarHistorico);
		...
		for (int i = 0; i <= 31; i++) 
		{
			casillas[i] = listaCasillasDeshacer.get(jugadaRevisarHistorico)[i];
			...
			try {
				...
				if (vPieza != null)
				{
//					System.out.println("[" + i + " Pieza :" + vPieza + " pgn: " + vPieza.getPosPgn());
					vPieza.setVisible(true);
					vPieza.setInd(i);
					vPieza.setPosPgn(casillas[i].getPgn());
					vPieza.setTouchable(Touchable.enabled);
...
//			}			
		...
		//System.out.println("jugadaRevisar < CantidadjugadaRevisar");	
			...
			removerFlechas();
			System.out.println("**** 5 deshacer.setTouchable(Touchable.enabled)");
			deshacer.setTouchable(Touchable.enabled);
		}
...
//*/
	...
	public void removerFlechas(){
		...
		esFlechaVisible = false;

		try{
//System.out.println("**************************** removerFlechas antes");
			siguiente.remove();
			atras.remove();
//System.out.println("**************************** removerFlechas despues");
...
private void accionTabla()
{
	...
	if ((Turno != Color.WHITE) && (Turno != Color.BLACK)) return;
	...
	if (decision)
	{
		...
		//System.out.println("YYYY AZUL 30 YYYY");
		colorAntesDelBlue = getTurno();

		if (colorAntesDelBlue == Color.BLACK)
		{
			colorAntesDelBlue = Color.WHITE;

		} else {

			colorAntesDelBlue = Color.BLACK;
		}
	...
	public void actualizarContadorT (String identificador, int contador)
		...
		if (skin3 == null) return;
		if (pack3 == null) return;

		if (contador<0)contador=0;

//		System.out.println("skin3.addRegions(pack3) " + skin3 + " " + pack3);
...
//			imagenReloj.setVisible(true);
			...
			} else if (selec4.equals("Personalizado")) {

				Dialog dialog = new Dialog("Introduzca Minutos", skin2) 
				{
					TextField txtUserTime;
					{
						txtUserTime = new TextField("", skin2);
						txtUserTime.setWidth(500);
						txtUserTime.setMaxLength(2);
						txtUserTime.setTextFieldFilter(new TextField.TextFieldFilter.DigitsOnlyFilter());
						this.getButtonTable().add(txtUserTime);
						button("Aceptar", "A");
					}
					...
					protected void result(Object object) 
					{
						if (object.equals("A")) 
						{
							String numMin = txtUserTime.getText();
							if (numMin != null) 
							{

								try {
										int minutosIntro = Integer.parseInt(numMin);
										if (minutosIntro<1 || minutosIntro>99)
										{
											minutosIntro = 5;
										}

										mostrarcontTiempo=true;
										tiempoJugadasBla=60*minutosIntro;
										tiempoJugadasNe=60*minutosIntro;
										imagenReloj.setZIndex(1);
										box5.setVisible(true);
										retardo.setVisible(true);
										inicializarRelojes();
									...
									//System.out.println("Error en numMin " + numMin);
									mostrarcontTiempo=true;
									tiempoJugadasBla=60*5;
									tiempoJugadasNe=60*5;
									imagenReloj.setZIndex(1);
									box5.setVisible(true);
									retardo.setVisible(true);
									inicializarRelojes();
							...
							} else { //Presiono Aceptar con el campo en blanco, entonces 5 min por defecto
	...
	private boolean decideTablas()
	{
		...
		ress = enviarMangoPaola("analizar");
		int valorAnalisis = 0;

		if (ress.size()>0)
		{
			valorAnalisis = Integer.parseInt(ress.get(0).split(":")[1].trim());
		}

		//System.out.println("***** decideTablas valorAnalisis=" + valorAnalisis);

		boolean decision =false;

		if(valorAnalisis > 300)
		{
			decision=true;
		}
		return decision;
	...
	private boolean colocarJugadaAlg(String pPgni , String pPgnf)
	{
		...
		//System.out.println("********************************************");
		//System.out.println("---------> colocarJugadaAlg <---------------");
		//System.out.println("********************************************");

		if (getTurno().equals(Color.BLUE))
		{
			//System.out.println("Turno Blue -> return");
			return false;
		}

		//System.out.println("---------> " + pPgni + " - " + pPgnf + "<---------------");
		//System.out.println("---------> colorOponente " + colorOponente);
		//System.out.println("---------> jugandovsMangoPaola " + jugandovsMangoPaola);
		//System.out.println("---------> jugadasAlg.size() " + jugadasAlg.size());
		//System.out.println("---------> jugadaHumano.get(0) " + jugadaHumano.get(0));
...
//System.out.println("---------> 7 <-----------------");
			...
			} else {	// end if jugandovsMangoPaola //Mango Paola esta apagado

//System.out.println("---------> 8 <-----------------");
				//System.out.println("estoy jugando solo");
...
//System.out.println("---------> 13 <-----------------");
				...
				} else if (getTurno().equals(Color.BLACK)) { //end if getTurno().equals(Color.WHITE)
...
//System.out.println("---------> 14 <-----------------");
					...
					if (!jugadaHumano.get(0).split("\\]")[0].contains("result"))
					{			
//System.out.println("---------> 15 <----------------- [" + temp + jugadaHumano.get(0).split("\\[")[1].split("\\]")[0] + "]");
...
//System.out.println("---------> 16.5 <-----------------");
		...
		} else if (colorOponente.equals(Color.BLACK))  { // else de colorOponente.equals(Color.WHITE) ---> Oponente es Negro
...
//System.out.println("---------> 22 <-----------------");
				...
				} else if (getTurno().equals(Color.BLACK)) { // end if getTurno().equals(Color.WHITE) ---> Turno de las Negras
...
//System.out.println("---------> " + (jugadasAlg.size()+1)+"."+ jugadaHumano.get(0).split("\\[")[2].split("\\]")[0] + " <-----------------");
...
//System.out.println("---------> 31 <-----------------");
		...
		} //end if colorOponente.equals(Color.WHITE)
		...
		} catch(Exception e){ System.out.println("Error en el pane " + e.getMessage());}
	...
	public void determinarTiempoBla(){
		//System.out.println("****************** >>>>determinarTiempoBla BLANCAS ");

		//System.out.println("lngTiempoMangoPaola " + lngTiempoMangoPaola);
		//System.out.println("tiempoJugadasBla " + tiempoJugadasBla);
		//System.out.println("retardoBlancas " + retardoBlancas);


		if (lngTiempoMangoPaola>0)
		{
			tiempoJugadasBla = tiempoJugadasBla - (int)lngTiempoMangoPaola;
			lngTiempoMangoPaola=0;
		}
		...
		if(tiempoJugadasBla>=60)
		{
			...
			if(tiempoJugadasBla>0)
			{
				...
				if(flagTiempo)
				{
					actualizarContadorT(cont4, 0);
					actualizarContadorT(cont3, 0);
//System.out.println(">>> ganartiempoX >> 3");
					ganartiempo(false);
					flagTiempo=false;
				}
			}
...
public void determinarTiempoNe(){
	//System.out.println("****************** >>>>determinarTiempoNe NEGRAS " + flagTiempo2);

		//System.out.println("lngTiempoMangoPaola " + lngTiempoMangoPaola);
		//System.out.println("tiempoJugadasNe " + tiempoJugadasNe);
		//System.out.println("retardoNegras " + retardoNegras);
	

		if (lngTiempoMangoPaola>0)
		{
			tiempoJugadasNe = tiempoJugadasNe - (int)lngTiempoMangoPaola;
			lngTiempoMangoPaola=0;
		}
	...
	{	
		...
		if(tiempoJugadasNe>0)
		{
			...
			if(flagTiempo2)
			{
				actualizarContadorT(cont2, 0);
				actualizarContadorT(cont1, 0);
//System.out.println(">>> ganartiempoX >> 4");
				ganartiempo(true);
				flagTiempo2=false;
			}
		}
...
public TextButtonStyle botonesReloj(Color estado){
 ...
 public void oprimirBotonReloj()
 {
	...
	//System.out.println("*** oprimirBotonReloj *** Turno: " + getTurno().toString());
	//System.out.println("*** oprimirBotonReloj *** colorOponente: " + colorOponente.toString());

	Color botonRelojTurno;

	if (colorOponente==Color.WHITE)
	{
		if (Turno == Color.WHITE)
		{
			botonRelojTurno = Color.BLACK;
			
		} else {

			botonRelojTurno = Color.WHITE;
		}		

	
	} else {

		botonRelojTurno = Turno;

	}

	//System.out.println("*** oprimirBotonReloj *** botonRelojTurno: " + botonRelojTurno.toString());
	//System.out.println("*** oprimirBotonReloj *** jugadas.size(): " + jugadas.size());

	TextButtonStyle style=	botonesReloj(botonRelojTurno);
	imagenBotones.setStyle(style);
...
private void guardarJuegos(int resultado ){
	...
	//System.out.println("******************* guardarJuegos *****************************");
	//System.out.println("******************* guardarJuegos *****************************");
	//System.out.println("******************* guardarJuegos *****************************");
...
private void iniciarRetardo(){
	...
	labelBlancas=new Image(txlabelBlancas);
	labelNegras=new Image(txlabelNegras);
	...
	if (imagenBotones != null)
	{
		imagenBotones.setVisible(false);
	}
	...
	if(flagReloj)
	{
		...
		contadorTiempo("3");
		contadorTiempo("4");
//System.out.println("Comprobar Tiempo 8");
		comprobartiempo();
		flagReloj=false;
	}
...
public void iniciarMotor(){
	...
	//System.out.println(">>>>>>>>>> iniciarMotor <<<<<<<<<<<<<<<<<");
