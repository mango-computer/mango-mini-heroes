...
public class mapaGeneralScreen extends ScreenManager {
	...
	private float subirPersonaje = 0;
...
//			System.out.println("ancho : "+ Configuracion.GetAtributo(pTilemap, "Mapa", "mapa", "Fondo"));
		...
		//
			...
			myActor.setPosition(posInicialX, posInicialY+subirPersonaje);
			return true;
		} catch (Exception e) {
...
//					System.out.println("cambiando a barco");
				...
				myActor.addAction(Actions.moveTo(estacion.get(estacionActual).getX(),
						estacion.get(estacionActual).getY()+subirPersonaje, duracion));
					...
					while (estacion.get(estacionActual).isEstacionar() == false) {
						...
						Timer.schedule(new Task() {
							...
							public void run() {
								...
								myActor.addAction(Actions.moveTo(estacion.get(estacionActual).getX(),
										estacion.get(estacionActual).getY()+subirPersonaje, duracion2));
							}
						}, duracion);
...
//			tximgEstacionRep = game.getManager().get("assets/Texturas/estacionRep.png");
			...
			for (int i = 0; i < c; i++) {
				if (batalla > i) {
					...
					imgEstacionPass[i].setPosition(posEstX[i] + (imgEstacionPass[i].getWidth()/2)-2, posEstY[i]-6);
					imgEstacionPass[i].setSize(24, 24);
					...
					imgEstacionRep[i].setPosition(posEstX[i] + (imgEstacionRep[i].getWidth()/2)-2, posEstY[i]-6);
					imgEstacionRep[i].setSize(24, 24);
